...
- «Продолжить просмотр»: позиция запоминается локально и (при входе) синхронизируется с сайтом; недосмотренные видео появляются в строке «Смотреть далее» лаунчера Android TV (Android 8+) и открываются оттуда сразу в плеере
...
| ExoPlayer: буфер 20–60 с, 1080p по умолчанию, выбранное качество фиксируется (без ABR-переключений) | старые SoC не тянут 4K H.264 и «замирают» при каждой смене рендиции; качество меняется в плеере и настройках |
...
│   ├── SettingsActivity# качество, шаг перемотки, аккаунт, история
│   ├── WatchNext       # строка «Смотреть далее» лаунчера (androidx.tvprovider)
│   └── LaunchActivity  # deep link tvsas://watch/{uuid} из лаунчера
